...
class StoryList {
  ...
  async addStory(user, newStory) {
    const results = await axios.post(`${BASE_URL}/stories`, {token: user.loginToken, story: newStory});
  ...
  /** Edits story data sent to API and in story list.
   * - user - the current instance of User who will post the story
   * - id of story to remove
   * - edited story to send to API
   */
  ...
  async editStory(user, storyId, editedStory) {
    ...
    user.favorites = user.favorites.map(function(story) {
      return story.storyId === storyId ? newStory : story;
    })

    user.ownStories = user.ownStories.map(function(story) {
      return story.storyId === storyId ? newStory : story;
    })
  }
}
